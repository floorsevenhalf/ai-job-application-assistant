...
export async function hybridMatchField(field:FieldDescriptor,ruleResult:MatchResult,options:HybridMatcherOptions):Promise<HybridMatchResult>{
  ...
  if(!result){
    ...
    try{result=await Promise.race([options.matcher.match(input),new Promise<never>((_,reject)=>{timer=setTimeout(()=>reject(new Error("ai_timeout")),timeoutMs);})]);cache?.set(key,result);}catch(error){return fallback(ruleResult,"failed",error instanceof Error&&error.message==="ai_timeout"?"timeout":error instanceof Error&&error.message.startsWith("invalid_semantic")?"invalid_response":"provider_error");}finally{if(timer)clearTimeout(timer);}
  }
